...
/**
 * Context implementation that delegates JNDI operations to 
 * {@link NamingContext} instances bound to currently running threads or
 * classloaders.  The {@link #getBoundContext} method, used to locate the 
 * appropriate Context instance, looks first for the current thread, then 
 * classloader in the bindings maintained in {@link ContextBindings}.
 * <p>
 * This implementation is limited to the "java:" namespace.  Operations on 
 * names or name strings that do not start with the "java:" URL header result
...
public class SelectorContext implements Context {
     ...
     * <p>
     * Tries to find a context bound to the current thread, then classloader.
     * If this is an initial context and there is no context bound to the 
     * current thread or classloader, a new <code>NamingContext</code> is 
     * created and returned.  The newly created context will be a 
     * {@link SynchronizedContext} if  
     * <code>SynchronizedContext.isSynchronized(env))</code>
     * 
     ...
     * <p>
     * If this is an initial context, <code>name</code> is returned; otherwise
     * the expected "java:" URL header is stripped and the remainder of the
     * name is returned.  If this is not an initial context and there is no 
     * "java:" URL header, a <code>NamingException</code> is thrown.
     * 
     ...
     * <p>
     * If this is an initial context, <code>name</code> is returned; otherwise
     * the expected "java:" URL header is removed and the remainder of the
     * name is returned.  If this is not an initial context and there is no 
     * "java:" URL header, a <code>NamingException</code> is thrown.
     * 
     * @param name The name to parse

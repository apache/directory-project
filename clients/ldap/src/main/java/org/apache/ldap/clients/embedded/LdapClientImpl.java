    ...
    implements LdapClient
{
    /**
     ...
     /**
      * Message number of the request being sent to the server
      */
     private int id = 1;
    ...
    private Socket m_socket = null ;
      ...
      * 
      * @return Port used to connect to the LDAP server
      */
     public int getPort()
     {
         return m_port ;
      ...
      * DN used to bind to the server
      * 
      * @return DN used to bind to the server
      */
      ...
      * Password of the DN entry used to bind to the server
      * 
      * @return Password of the DN entry used to bind to the server
      */
      ...
      * 
      * @return True if the LDIF format version 3 compliant
      */
      ...
      * 
      * @return True if verbose has been turned on
      */
    ...
    {
        InetAddress l_address = InetAddress.getByName( m_host )  ;
        m_socket = new Socket( l_address, m_port ) ;
    ...
    public LdapClientResponse ldapBind() throws IOException
    {
        ...
        l_request.setName( m_bindDn ) ;
        l_request.setSimple( true ) ;
        l_request.setCredentials( m_password.getBytes() ) ;
        l_request.setVersion3( m_isVersion3 ) ;
        ...
        l_encoder.encodeBlocking( null, m_socket.getOutputStream(), l_request );
        id++;
        
        // Retrieve the response back from server to my last request.
        BindResponse l_bindResponse = 
            ( BindResponse ) l_decoder.decode( null, m_socket.getInputStream() );
        LdapClientResponse l_clientResponse = new LdapClientResponse() ;
        
        l_clientResponse.setResultCode (
            l_bindResponse.getLdapResult().getResultCode().getValue() ) ;
        l_clientResponse.setErrorMessage(
            l_bindResponse.getLdapResult().getErrorMessage() ) ;
        l_clientResponse.setResultCodeString(
            l_bindResponse.getLdapResult().getResultCode().toString() ) ;
    ...
    {
        UnbindRequest l_request = new UnbindRequestImpl( id ) ;
    ...
    public LdapClientResponse ldapDelete ( String a_deleteDn ) throws IOException
    {
        ...
        // Retrieve the response back from server to my last request.
        DeleteResponse l_deleteResponse = 
             ( DeleteResponse ) l_decoder.decode( null, m_socket.getInputStream() );
        LdapClientResponse l_clientResponse = new LdapClientResponse() ;        
    
        l_clientResponse.setResultCode (
            l_deleteResponse.getLdapResult().getResultCode().getValue() ) ;
        l_clientResponse.setErrorMessage(
            l_deleteResponse.getLdapResult().getErrorMessage() ) ;
        l_clientResponse.setResultCodeString(
            l_deleteResponse.getLdapResult().getResultCode().toString() ) ;
            
        return l_clientResponse ;
    }
    
    
    public LdapClientResponse ldapAdd ( StringBuffer a_entry )
       throws IOException
    {
        // Setup the ASN1 Enoder and Decoder
        MessageEncoder l_encoder = new MessageEncoder();
        ...
        AddRequest l_request = new AddRequestImpl( id )  ;
        id++;
        ...
        try
        {
            ...
            l_parser.parse( l_attributes, a_entry.toString() ) ;
        ...
        AddResponse l_addResponse =
            ( AddResponse ) l_decoder.decode( null, m_socket.getInputStream() ) ;
        LdapClientResponse l_clientResponse = new LdapClientResponse() ;
    
        l_clientResponse.setResultCode (
            l_addResponse.getLdapResult().getResultCode().getValue() ) ;
        l_clientResponse.setErrorMessage(
    ...
    public LdapClientResponse ldapModify ( StringBuffer a_entry )
        throws IOException 
    {
        LdapClientResponse l_clientResponse = new LdapClientResponse () ;
        ...
        ModifyResponse l_modifyResponse =
          ( ModifyResponse ) l_decoder.decode( null, m_socket.getInputStream() ) ;

        l_clientResponse.setResultCode (
            l_modifyResponse.getLdapResult().getResultCode().getValue() ) ;
        l_clientResponse.setErrorMessage(
            l_modifyResponse.getLdapResult().getErrorMessage() ) ;
        l_clientResponse.setResultCodeString(
            l_modifyResponse.getLdapResult().getResultCode().toString() ) ;
